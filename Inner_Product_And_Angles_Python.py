...
def distance(x0, x1):
    ...
    """
    distance_vector = np.subtract(x0, x1)
    return np.power(np.dot(distance_vector, distance_vector), 0.5)
...
def angle(x0, x1):
    ...
    """
    x_dot_y = np.dot(x0, x1)
    x_mag = np.power(np.dot(x0,x0), 0.5)
    y_mag = np.power(np.dot(x1,x1), 0.5)
    angle = np.arccos(x_dot_y/(x_mag * y_mag))
    return angle
...
for i in mean_images.keys():
    for j in mean_images.keys():
        ...
        AG[i, j] = angle(mean_images[i].ravel(), mean_images[j].ravel()) #flattens the nested structure
