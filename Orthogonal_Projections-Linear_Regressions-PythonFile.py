...
#     print(x, b, x.shape, b.shape)
    project_matrix = np.outer(b,b) / np.dot(b,b)
    x = x.reshape((-1, 1))
#     print(project_matrix, x, project_matrix.shape, x.shape)
#     ravel converts/ flattens matrix into (D,) ndarray
    return np.matmul(project_matrix, x).ravel()
...
#     return p
    projection_matrix = np.matmul(np.matmul(B, np.linalg.inv(np.matmul(B.T, B))), B.T)
    projection_vector = np.matmul(projection_matrix, x)
