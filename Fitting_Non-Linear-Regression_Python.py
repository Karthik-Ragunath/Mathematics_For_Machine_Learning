...
def dfdmu (x,mu,sig) :
#     return f(x, mu, sig) * -1 * np.sqrt(np.true_divide(2, np.pi)) * np.true_divide(1, np.pow(sig, 3)) * np.exp(-(x-mu)**2/(2*sig**2)) * (x - mu)
    return f(x, mu, sig) * np.true_divide(1, np.power(sig, 2)) * (x - mu)
#     return f(x, mu, sig) * (1 / sig ** 2) * (x - mu)

# Finally in this cell, the derivative with respect to σ.
# === COMPLETE THIS FUNCTION ===
def dfdsig (x,mu,sig) :
#     return f(x, mu, sig) * -1 * np.sqrt(np.true_divide(2, np.pi)) * np.true_divide(1, np.pow(sig, 4)) * np.exp(-(x-mu)**2/(2*sig**2)) * np.pow((x - mu), 2)
    return f(x, mu, sig) * ((np.true_divide(1, np.power(sig, 3)) * np.power((x - mu), 2)) - np.true_divide(1, sig))
#     return f(x, mu, sig) * (((1 / sig ** 3) * ((x - mu) ** 2)) - (1 / sig))
