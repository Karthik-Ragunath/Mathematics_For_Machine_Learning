...
def gsBasis(b_basis):
    bear_basis = np.array(b_basis, dtype=np.float)
    for i in range(bear_basis.shape[1]):
        for j in range(i):
            bear_basis[:, i] = bear_basis[:, i] - bear_basis[:, i] @ bear_basis[:, j] * bear_basis[:, j]
        if norm(bear_basis[:, i]) > very_small_number:
            bear_basis[:, i] = bear_basis[:, i] / norm(bear_basis[:, i])
        else:
            bear_basis[:, i] = np.zeros_like(bear_basis[:, i])
    return bear_basis
